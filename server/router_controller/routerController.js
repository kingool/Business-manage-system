...
class RouterController {
  ...
  showProduct(req,res){
    ...
    let sql = "SELECT `p`.`name` , `p`.`count`  FROM  `product`  AS  `p`  INNER  JOIN  `type`  AS  `t`  ON  `p`.`type` = `t`.`type_id` AND `p`.`user_id` = :userId"
    //SELECT p.name, p.countFROM  product  AS  p  INNER  JOIN  type  AS  t ON  p.type = t.type_id  AND p.user_id = '_b1591945612651'  AND t.type_id = '_ty1591946862100'
    //根据类型
    if (req.query.typeId) {
    ...
    api.query(sql, condition).then(result => {
      res.send({msg: '查询商品数据成功', code: 1100, result});
    }).catch(err => {
      console.log('err ==> ', err);
      res.send({msg: '查询商品数据失败', code: 1101});
    })
  }
